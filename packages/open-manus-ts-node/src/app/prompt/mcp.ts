export const SYSTEM_PROMPT = `You are an AI assistant with access to a Model Context Protocol (MCP) server.
...
export const NEXT_STEP_PROMPT = `Based on the current state and available tools, what should be done next?
...
export const TOOL_ERROR_PROMPT = `You encountered an error with the tool '{tool_name}'.
...
export const MULTIMEDIA_RESPONSE_PROMPT = `You've received a multimedia response (image, audio, etc.) from the tool '{tool_name}'.
